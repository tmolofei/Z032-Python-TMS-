...
# today = date.today().strftime('%d/%m/%Y').split("/")  # if use current day
...
diff_years = (int(user_date[2]) - int(date_of_birth[2])) * 365
diff_months = (int(user_date[1]) - int(date_of_birth[1])) * 30
diff_days = int(user_date[0]) - int(date_of_birth[0])
